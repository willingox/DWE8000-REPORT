	...
	function openNewWindow() {
		window.opener=null; 
		window.close(); 	
		window.open('temp.html','','width=1024,height=768,left=0,top=0,titlebar=no,menubar=no,toolbar=no,location=no,status=no,scrollbars=yes,resizable=yes');
		return false;
	}
	
	function btn_login_over(o){
		$(o).css("background-position","-70px 0");
	}
	
	function btn_login_down(o){
		$(o).css("background-position","-140px 0");
	}
	
	function btn_login_out(o){
	...
	function airBalloon(balloon){
		...
		$(window).resize(function(){
			$(balloon).each(function(){
				$(this).stop().animate({top:rand(40,viewHeight*0.5),left:rand(10,viewWidth-$(this).width())},1000,function(){fly(this);});
			});
		});
