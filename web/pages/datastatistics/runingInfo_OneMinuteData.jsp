...
function daysDiff() {
	var startDateDisp = document.runingInfo_OneMinuteDataForm.startDateDisp.value;
	var endDateDisp = document.runingInfo_OneMinuteDataForm.endDateDisp.value;
...
function query() {
		...
		var msg = [];
		if (document.runingInfo_OneMinuteDataForm.startDateDisp.value == '') 
			msg.push('起始日期');
		
		if (document.runingInfo_OneMinuteDataForm.endDateDisp.value == '') 
			msg.push('终止日期');
	
		if(document.runingInfo_OneMinuteDataForm.endDateDisp.value < document.runingInfo_OneMinuteDataForm.startDateDisp.value)
			msg.push('正确时间');
            if(daysdiff>2)
			msg.push('时间间隔小于1天');
		if (msg.length > 0) {
		 ...
		 runingInfo_OneMinuteDataForm.method.value = 'show';
             runingInfo_OneMinuteDataForm.submit();
	...
	var  flagVal = $("input[name='flag']:checked").val();
		runingInfo_OneMinuteDataForm.flag.value = flagVal;
		...
		if(document.runingInfo_OneMinuteDataForm.endDateDisp.value < document.runingInfo_OneMinuteDataForm.startDateDisp.value)
			msg.push('正确时间');
	          if(runingInfo_OneMinuteDataForm.flag.value=='')
		  {
		...
		if (msg.length > 0) {
          ...
          if (window.showModalDialog!=null)//IE判断
           {
          window.showModalDialog('${pageContext.request.contextPath}/datastatistics/runingInfo_SelectOneMinuteData.do?key='+key, window, 'dialogHeight:600px;dialogWidth:500px;help:no;status:no');	
          }
      
          else{
          window.open('${pageContext.request.contextPath}/datastatistics/runingInfo_SelectOneMinuteData.do?key='+key,'','width=500,height=600,status=no,modal=yes');
          }
...
<BODY scroll="auto">
<!--如有页面需要-->
	<html:form action="runingInfo_OneMinuteData" method="post">
	  ...
	  <TD width="10"></TD>
			<TD>开始日期      </TD>
					...
					<TD width="25"></TD>
								 ...
								 <% 
                                       out.print(" <input id = 'flag1'  type='radio' name='flag' value="+1+" "+request.getAttribute("check0")+" />"+"1分钟");
                                       out.print(" <input id = 'flag2'  type='radio' name='flag' value="+2+" "+request.getAttribute("check1")+" />"+"10分钟");
                                       out.print(" <input id = 'flag3'  type='radio' name='flag' value="+3+" "+request.getAttribute("check2")+" />"+"15分钟");
                                       out.print(" <input id = 'flag4'  type='radio' name='flag' value="+4+" "+request.getAttribute("check3")+" />"+"30分钟"); 
                                       out.print(" <input id = 'flag5'  type='radio' name='flag' value="+5+" "+request.getAttribute("check4")+" />"+"1小时");   
                                     %>	
			 <TD width="30"></TD>
			...
			<tr>
			  ...
			  <tr>			     
				...
				<ec:table 
					...
					items="resulttu"
					var="bean"
					action="${pageContext.request.contextPath}/datastatistics/runingInfo_OneMinuteData.do?method=show"
					retrieveRowsCallback="process"
					sortRowsCallback="process"
					filterRowsCallback="process"
					xlsFileName="runingInfo_OneMinuteData.xls"
                              showTitle="ture"
					listWidth ="100%"
					listHeight="400"
					width="100%"
