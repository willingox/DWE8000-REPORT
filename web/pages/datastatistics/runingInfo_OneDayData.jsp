...
padding:5px; 
	...
	<script type="text/javascript">
function daysDiff() {
	var startDateDisp = document.runingInfo_OneDayDataForm.startDateDisp.value;
	var endDateDisp = document.runingInfo_OneDayDataForm.endDateDisp.value;
...
function query() {
		...
		var msg = [];
		if (document.runingInfo_OneDayDataForm.startDateDisp.value == '') 
			msg.push('起始日期');
		
		if (document.runingInfo_OneDayDataForm.endDateDisp.value == '') 
			msg.push('终止日期');
	
		if(document.runingInfo_OneDayDataForm.endDateDisp.value < document.runingInfo_OneDayDataForm.startDateDisp.value)
			msg.push('正确时间');
            if(daysdiff>32)
			msg.push('时间间隔小于一个月');
		...
		if (msg.length > 0) {
		 ...
		 runingInfo_OneDayDataForm.method.value = 'show';
             runingInfo_OneDayDataForm.submit();
	...
	function select() {
		...
		if (msg.length > 0) {
          ...
          var key=3; 
         // window.showModalDialog('${pageContext.request.contextPath}/datastatistics/runingInfo_SelectOneDayData.do?key='+key, window, 'dialogHeight:600px;dialogWidth:500px;help:no;status:no');	
	 if (window.showModalDialog!=null)//IE判断
          ...
          window.open('${pageContext.request.contextPath}/datastatistics/runingInfo_SelectOneDayData.do?key='+key,'','width=500,height=600,status=no,modal=yes');
          } 
	}
...
<BODY scroll="auto">
<!--如有页面需要-->
	<html:form action="runingInfo_OneDayData" method="post">
	...
	<TABLE>	
	  <TD width="10"></TD>
			<TD>开始日期      </TD>
					...
					<TD>结束日期      </TD>
					<TD id="Select" >										
						<html:text  property="endDateDisp" styleId="startDateId" styleClass="dateInput" size="20" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowWeek:true,isShowClear:true,readOnly:true,firstDayOfWeek:1,maxDate:'2022-12-28'})" title="点击选择结束日期" />
					</TD>	
					<TD width="25"></TD>
					<%-- <TD style="color:#000000">时间间隔</TD>

					<TD id="SelectWind">
					<html:select property="flag"
							value="${runingInfo_OneDayDataForm.flag}"
							styleClass="formDataFormSelect"
							style="width:120px;border-width:1;">

							<logic:present name="queryTime">
								<html:options collection="queryTime" labelProperty="name"
									property="id" />
							</logic:present>
					</html:select></TD> --%>
			 <TD width="30"></TD>
			...
			<tr>
			  ...
			  <tr>			     
				...
				<ec:table 
					...
					items="resulttu"
					var="entity"
					action="${pageContext.request.contextPath}/datastatistics/runingInfo_OneDayData.do?method=show"
					retrieveRowsCallback="process"
					sortRowsCallback="process"
					filterRowsCallback="process"
					xlsFileName="runingInfo_OneDayData.xls"
                              showTitle="ture"
					listWidth ="100%"
					listHeight="400"
					width="100%"
					...
					<ec:column  headerSpan="0" property="saveTime" title="时间" cell="date" format="yyyy-MM-dd" headerClass="ecTableHead" styleClass="ecTableBody"  width="200" style="text-align:center"/>
