		...
		if (!StringUtils.isEmpty(request.getParameter("isFirst"))) {
			...
			for(int i=0;i<size;i++){
			    ...
			    for(int m=0;m<resultGen.size();m++){
						...
						if(Integer.parseInt(arr[i].substring(arr[i].lastIndexOf("-")+1,arr[i].length()))==resultGen.get(m).getId()){
							String nameWind=resultGen.get(m).getName();
				...
				if(str.length()<11){	
					for(int i=0;i<resultGen.size();i++){
						...
						for(int j=0;j<result.size();j++){
							...
							if(resultGen.get(i).getId()==result.get(j).getEquipId()){
								countWind=countWind+1;
							}
						...
						String nameWind=resultGen.get(i).getName();
						dataStatisticsDataVo.setName(nameWind);	
			...
			if(resultCount.size()!=0){	
				for(int i=0;i<resultCount.size();i++){
					freCount+=resultCount.get(i).getCount();
				}
			System.out.println(freCount);
				freCount=freCount/resultCount.size();
				freCount  =   (new   BigDecimal(freCount)).setScale(2,   BigDecimal.ROUND_HALF_UP).doubleValue();
			}
